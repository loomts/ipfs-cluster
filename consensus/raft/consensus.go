...
func (cc *Consensus) redirectToLeader(ctx context.Context, method string, arg interface{}) (bool, error) {
	...
	for i := 0; i <= cc.config.CommitRetries; i++ {
		...
		logger.Debugf("redirecting %s to leader: %s", method, leader.String())
...
func (cc *Consensus) AddPeer(ctx context.Context, pid peer.ID) error {
	...
	var finalErr error
	for i := 0; i <= cc.config.CommitRetries; i++ {
		logger.Debugf("attempt #%d: AddPeer %s", i, pid.String())
		if finalErr != nil {
		...
		logger.Infof("peer added to Raft: %s", pid.String())
		break
	}
	return finalErr
}
...
func (cc *Consensus) RmPeer(ctx context.Context, pid peer.ID) error {
	...
	var finalErr error
	for i := 0; i <= cc.config.CommitRetries; i++ {
		logger.Debugf("attempt #%d: RmPeer %s", i, pid.String())
		if finalErr != nil {
		...
		logger.Infof("peer removed from Raft: %s", pid.String())
		break
	}
	return finalErr
}
