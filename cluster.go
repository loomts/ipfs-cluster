...
func (c *Cluster) PinUpdate(ctx context.Context, from api.Cid, to api.Cid, opts api.PinOptions) (api.Pin, error) {
	...
	if opts.UserAllocations != nil {
		existing.Allocations = opts.UserAllocations
		existing.UserAllocations = opts.UserAllocations
	}
...
func ECExtraMetaData(metadata map[string]string) (map[int][]sharding.ECBlockMeta, []int, error) {
	...
	// bmeta used to recover each block
	bmeta := make(map[int][]sharding.ECBlockMeta)
	for k, v := range metadata {
		if v == "" {
			m, err := sharding.UnmarshalECBlockMeta(k)
			if err != nil {
				return nil, nil, err
			}
			if bmeta[m.ShardNo] == nil {
				bmeta[m.ShardNo] = make([]sharding.ECBlockMeta, 0, 256)
			}
			bmeta[m.ShardNo] = append(bmeta[m.ShardNo], m)
		} else {
	...
	for _, m := range bmeta {
		sort.Slice(m, func(i, j int) bool {
			return m[i].BlockNo < m[j].BlockNo
		})
	}
	dss := make([]int, len(dShardSize))
	for i, v := range dShardSize {
		dss[i] = v
	}
	...
	for i := 0; i < len(shards); i++ {
		...
		if idx >= len(dShardSize) {
			go func() {
				...
				mapDir := files.NewMapDirectory(map[string]files.Node{parityAddParams.Name: files.NewBytesFile(shards[i])})
		...
		// 		errCh <- err
		// 	}
		// 	prev.PinOptions.UserAllocations = []peer.ID{p}

		// 	_, err = c.PinUpdate(ctx, prev.Cid, prev.Cid, prev.PinOptions)
		// 	if err != nil {
		// 		errCh <- err
		// 	}
		// 	logger.Infof("update data shard(%s) allocation to %v", shardCid, prev.Allocations)
		// }(needRepin[i])
	}
	wg.Wait()
	close(errCh)
