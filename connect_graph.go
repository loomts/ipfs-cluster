...
func (c *Cluster) recordClusterLinks(cg *api.ConnectGraph, p string, peers []api.ID) (bool, api.ID) {
	...
	for _, id := range peers {
		if id.Error != "" {
			logger.Debugf("Peer %s errored connecting to its peer %s", p, id.ID.String())
			continue
		}
		if id.ID.String() == p {
			selfConnection = true
...
func (c *Cluster) recordIPFSLinks(cg *api.ConnectGraph, pID api.ID) {
	...
	if pID.IPFS.Error != "" { // Only setting ipfs connections when no error occurs
		logger.Warnf("ipfs id: %s has error: %s. Skipping swarm connections", ipfsID.String(), pID.IPFS.Error)
		return
	}
	...
	if _, ok := cg.IPFSLinks[pid]; ok {
		logger.Warnf("ipfs id: %s already recorded, one ipfs daemon in use by multiple cluster peers", ipfsID.String())
	}
